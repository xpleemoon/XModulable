...
以IM业务模块为例，从源码的角度看下它们是实现这套思路的。在common层把IM业务模块想要暴露给其他业务模块的服务（`LiveService`）进行了暴露，同时在common层暴露了一个`LiveModule`（live业务模块的服务容器，承载了`LiveService `）。这样的话，上层业务就可以通过[XModulable SDK](https://github.com/xpleemoon/XModulable)获取到`LiveModule`，然后通过`LiveModule`承载的服务进行调用。
